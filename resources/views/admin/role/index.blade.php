            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-1">
                            @if(auth()->user()->hasPermissionTo('Add Role'))
                                <a href="{{ route('admin.role.create') }}" class="btn btn-block btn-primary"><i class="fas fa-plus"></i> Add</a>
                            @endif
                        </div>
                    </div>
                    </div>
                    ...
                    <div class="card-body" style="overflow: auto;">
                        <table id="dataTable" class="table table-bordered table-hover" data-url="{{route('admin.role.getRoles')}}" data-destroy-url="{{route('admin.role.destroy')}}" data-change-status-url="{{route('admin.role.changeStatus')}}" data-export-csv-url="{{route('admin.role.export', ['type' => 'excel'])}}">
                            <thead>
                                <tr>
                                    <th>S.no.</th>
                                    <th>Name</th>
...
<!-- /.content -->
@php
$viewPermission = auth()->user()->hasPermissionTo('View Role');
$editPermission = auth()->user()->hasPermissionTo('Edit Role');
$deletePermission = auth()->user()->hasPermissionTo('Delete Role');
@endphp
...
<script src="{{ asset('public/js/role/index.js') }}"></script>
@endpush
