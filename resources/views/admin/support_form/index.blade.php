                ...
                <div class="card">
                    ...
                    <div class="card-body" style="overflow: auto;">
                        <table id="dataTable" class="table table-bordered table-hover" data-url="{{route('admin.city.getCities')}}" data-destroy-url="{{route('admin.city.destroy')}}" data-change-status-url="{{route('admin.city.changeStatus')}}" data-export-csv-url="{{route('admin.city.export', ['type' => 'excel'])}}">
                            ...
                            <tbody>
                                @foreach($supportform as $key => $item)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $item->name }}</td>
                                    <td>{{ $item->email }}</td>
                                    ...
                                    <td>{{ $item->subject }}</td>
                                    <td>{{ $item->message }}</td>
                                    <td><img src="{{ asset($item->image) }}" width="50" height="50"></td>
                                    <td>
                                        @if(auth()->user()->hasPermissionTo('Edit Support Form'))
                                            <a href="{{ route('admin.support-form.edit', ['id' => $item->id]) }}" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i></a>
                                        @endif
                                    </td>
                                </tr>
                                @endforeach
